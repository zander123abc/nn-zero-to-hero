(() => {
  ...
  const TITLES = {overview:"Command",cashflow:"Plan",portfolio:"Invest",learn:"Learn",settings:"More"};
  ...
  function normalizeVault(){vault.snaps=Array.isArray(vault.snaps)?vault.snaps:[];vault.reads=vault.reads&&typeof vault.reads==="object"?vault.reads:{};vault.ui=vault.ui&&typeof vault.ui==="object"?vault.ui:{};vault.ui.goals=Array.isArray(vault.ui.goals)?vault.ui.goals:[];vault.dismissed=Array.isArray(vault.dismissed)?vault.dismissed:[];}
  ...
  async function create(p){const s=crypto.getRandomValues(new Uint8Array(16));key=await derive(p,s);vault={snaps:[],reads:{},dismissed:[],ui:{tab:"overview",goals:[]}};const e=await encrypt(key,vault);localStorage.setItem(STORE,JSON.stringify({v:2,it:ITER,s:b64(s),iv:e.iv,c:e.c,t:new Date().toISOString()}))}
  ...
  function goalValues(s){
    const local=Array.isArray(vault?.ui?.goals)?vault.ui.goals:[],m=s?.metrics||{},goals=[...local];
    if(num(m.emergency_fund_target)!==null&&!goals.some(g=>String(g.name).toLowerCase().includes('emergency'))){goals.unshift({id:'report-emergency',name:'Emergency reserve',current:num(m.emergency_fund_balance),target:num(m.emergency_fund_target),monthly:null,target_date:null,horizon:'Now',priority:1,source:'report'})}
    return goals.sort((a,b)=>(num(a.priority)??99)-(num(b.priority)??99));
  }
  function renderPlan(s){
    const m=s?.metrics||{},cash=num(m.immediately_available_cash),due=num(m.near_term_obligations),gap=cash!==null&&due!==null?Math.max(0,due-cash):null;
    $("protectCash").textContent=money(cash);$("protectDue").textContent=money(due);$("protectGap").textContent=money(gap);
    const goals=goalValues(s),list=$("goalList");list.textContent="";
    goals.forEach(g=>{const cur=num(g.current),target=num(g.target),ratio=cur!==null&&target&&target>0?Math.max(0,Math.min(1,cur/target)):0,card=document.createElement('article');card.className='goal-card';card.innerHTML=`<div class="goal-top"><div><small>${escapeText(g.horizon||'Future')} · Priority ${escapeText(g.priority||'—')}</small><h3>${escapeText(g.name||'Goal')}</h3></div><strong>${target?Math.round(ratio*100)+'%':'Planning'}</strong></div><div class="mini-track"><div class="mini-fill"></div></div><div class="goal-meta"><span>${money(cur)} of ${money(target)}</span><span>${num(g.monthly)!==null?money(g.monthly)+'/mo':'Pace needed'}</span><span>${g.target_date?dateShort(g.target_date):'Date needed'}</span></div>`;card.querySelector('.mini-fill').style.width=`${ratio*100}%`;if(g.source!=='report'){const del=document.createElement('button');del.type='button';del.className='goal-delete';del.textContent='Remove';del.setAttribute('aria-label',`Remove ${g.name} goal`);del.addEventListener('click',()=>removeGoal(g.id));card.appendChild(del)}list.appendChild(card)});
    if(!goals.length)list.innerHTML='<div class="signal WATCH"><span class="signal-mark"></span><div><strong>No goals modeled</strong><p>Add a target and date to calculate pacing without moving any money.</p></div></div>';
    const planned=goals.map(g=>num(g.monthly)).filter(v=>v!==null).reduce((a,b)=>a+b,0);$("monthlyGoalLoad").textContent=planned?`${money(planned)}/mo`:"Needs goal pacing";
    $("goalConflict").textContent=gap>0?'Required obligations currently outrank flexible goals.':'No verified near-term cash shortfall.';$("goalConflict").className=`signal ${gap>0?'CHANGE':'HEALTHY'}`;
  }
  async function removeGoal(id){vault.ui.goals=vault.ui.goals.filter(g=>g.id!==id);await save();renderPlan(latest())}
  ...
  function renderPortfolio(s){const p=s?.portfolio||{},m=s?.metrics||{},items=[['U.S. equity',num(p.us_equity_pct),'var(--accent)'],['International',num(p.international_equity_pct),'var(--cyan)'],['Crypto',num(p.crypto_pct),'var(--purple)']].filter(x=>x[1]!==null);let used=items.reduce((a,x)=>a+x[1],0),ok=items.length>0&&used<=100.5;if(ok&&used<99.9)items.push(['Other / cash',Math.max(0,100-used),'var(--surface-3)']);let cursor=0,stops=[];for(const [,v,c] of items){stops.push(`${c} ${cursor}% ${cursor+v}%`);cursor+=v}$("portfolioDonut").style.background=ok?`conic-gradient(${stops.join(',')})`:'var(--surface-3)';$("portfolioTotal").textContent=ok?money(m.invested_assets,true):'Hidden';$("portfolioLegend").textContent="";if(ok)items.forEach(([label,v,c])=>{const r=document.createElement('div');r.className='legend-row';r.innerHTML=`<span class="legend-swatch"></span><span>${label}</span><strong>${pct(v)}</strong>`;r.querySelector('.legend-swatch').style.background=c;$("portfolioLegend").appendChild(r)});else $("portfolioLegend").innerHTML='<p class="data-warning">Allocation hidden: components do not reconcile to a valid denominator.</p>';$("portfolioSource").textContent=`Integrated planning allocation · denominator ${money(m.invested_assets)} · ${s?.data_quality?.manual_data_stale?'includes stale/manual values':'linked/report data'}`;const risks=[['Speculative sleeve',num(p.speculative_sleeve_pct),15,'risk'],['Crypto',num(p.crypto_pct),7,'risk'],['QQQM',num(p.qqqm_pct),30,''],['Leveraged ETF',num(p.leveraged_etf_pct),2,'warn']];$("riskBars").textContent="";risks.forEach(([label,v,max,cls])=>{const r=document.createElement('div');r.className='risk-row';const headroom=v===null?'unknown':`${pct(Math.max(0,max-v))} headroom`;r.innerHTML=`<div class="risk-top"><span>${label}</span><strong>${pct(v)} · ${headroom}</strong></div><div class="risk-track"><div class="risk-fill ${cls}"></div></div>`;r.querySelector('.risk-fill').style.width=`${v===null?0:Math.min(100,v/max*100)}%`;$("riskBars").appendChild(r)});const sb=$("specBadge"),sv=num(p.speculative_sleeve_pct);sb.textContent=sv===null?'Spec unknown':`${pct(sv)} / 15% ceiling`;sb.className='pill '+(sv!==null&&sv>15?'bad':sv!==null&&sv>10?'warn':sv!==null?'good':'')}
  ...
  async function deleteSnapshot(generatedAt){
    ...
    if(!vault.dismissed.includes(generatedAt))vault.dismissed.push(generatedAt);
    await save();
    render();
    $("privacyMsg").textContent=`Deleted the ${label} snapshot. It will stay dismissed until a newer feed arrives; vault and sync pairing were preserved.`;
  }
  ...
  function render(){const s=latest();renderStatus(s);renderMetrics(s);renderActions(s);renderLiquidity(s);renderPlan(s);renderSpend(s);renderPortfolio(s);renderRetirement(s);renderReading(s?.readings||[]);renderHistory();renderTrend();renderSyncUI()}
  ...
  async function syncNow(silent=false){
    ...
    try{
      ...
      stage="validate";
      valid(snap);
      if(vault.dismissed.includes(snap.generated_at)){
        vault.sync.last_sync=new Date().toISOString();await save();render();
        if(!silent)$("syncMsg").textContent="Feed verified; this snapshot remains deleted on this device.";
        return;
      }
      const newest=latest();
      if(!newest||new Date(snap.generated_at)>new Date(newest.generated_at)){stage="import";vault.dismissed=vault.dismissed.filter(t=>new Date(t)>=new Date(snap.generated_at));await importSnap(snap)}
      stage="save";
  ...
  $("goalForm").addEventListener('submit',async e=>{e.preventDefault();const name=$("goalName").value.trim(),target=Number($("goalTarget").value),current=Number($("goalCurrent").value||0),monthly=$("goalMonthly").value===''?null:Number($("goalMonthly").value),targetDate=$("goalDate").value||null;if(!name||!Number.isFinite(target)||target<=0)return $("goalMsg").textContent="Enter a goal name and positive target.";vault.ui.goals.push({id:crypto.randomUUID(),name,target,current:Number.isFinite(current)?current:0,monthly:Number.isFinite(monthly)?monthly:null,target_date:targetDate,horizon:$("goalHorizon").value,priority:vault.ui.goals.length+2,source:'local'});await save();e.target.reset();$("goalMsg").textContent="Goal saved inside the encrypted local vault. No money was moved.";renderPlan(latest())});
